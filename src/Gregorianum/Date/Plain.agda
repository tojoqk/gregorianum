...
fromPlain? (y , m , d) | yes (suc width , ym , p) with D.fromPlain? {width} d
fromPlain? (y , m , d) | yes (suc width , ym , p) | yes (pos cursor , proof) = yes ((ym - pos cursor) , plain p proof)
fromPlain? (y , m , d) | yes (suc width , ((y₁ Y.⟨ yp₁ ⟩ ) YM.- month₁ ⟨ has-days₁ ⟩) , YM.plain (Y.plain refl) mp₁) | no ¬q = no h
  where
